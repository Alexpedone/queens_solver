...
fn parse_line_to_grid(line_str:&String) -> Vec<Case>{
    ...
    line_vec.push(Case{color:line_str[0..commas_index[0]].parse().unwrap(), queen:false,});

    for i in 0..commas_index.len() - 1{
        line_vec.push(Case{color:line_str[commas_index[i]+1..commas_index[i+1]].parse().unwrap(),queen:false,});
    };
    line_vec.push(Case{color:line_str[commas_index[commas_index.len() - 1] + 1..].parse().unwrap(), queen:false,});
    return line_vec
}
...
fn authorized_queen(grid:&Grid,x:&usize,y:&usize) -> bool{
    for i in 0..grid.cases.len() {
        if !check_queen_in_line(grid,i) || !check_queen_in_column(grid,i)||!check_queen_in_color(grid,i)||!check_queen_in_neighbor(grid, x, y){
            return false
        }
    }
    true
